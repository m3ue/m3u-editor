...
class BuildEpgMapCandidatesJob implements ShouldQueue
{
    ...
    {
        $map = EpgMap::with(['epg', 'playlist'])->find($this->epgMapId);
        ...
        DB::transaction(function () use ($map, $channels, $epg, $settings, $matcher, $prefetched): void {
            // Replace any prior run for this map so the review surface
            // always reflects the currently unresolved channels.
            $map->candidates()->delete();
            ...
            // Insert in chunks to avoid parameter-count limits on very large
            // playlists (184 pages × 20 = ~3.7k unresolved channels is normal).
