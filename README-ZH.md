...
| 使用场景 | 文件 | 说明 |
| --------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **模块化部署** | [docker-compose.proxy.yml](./docker-compose.proxy.yml) | ⭐⭐ 强烈推荐！ 将编辑器、代理服务及 Redis 拆分为独立容器运行。这种方案控制粒度更细，且支持硬件加速（需配合外部代理）。你也可以根据需要轻松扩展 Postgres 或 NGINX 容器。|
...
> 无论是编写文档、修复 Bug，还是开发新功能，我们都很欢迎！❤️
