...
>
    ...
    <button
        ...
        :class="snapshot.degraded ? 'text-gray-400 dark:text-gray-500' : (hasActivity ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400')"
    ...
    >
        ...
        <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2.5 dark:border-white/10">
            <span class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Queue') }}</span>
        ...
        <div class="max-h-96 overflow-y-auto">
            ...
            <template x-if="snapshot.batches.length > 0">
                <div class="border-b border-gray-100 px-4 py-2 dark:border-white/10">
                    <p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{{ __('Batches') }}</p>
                    <template x-for="batch in snapshot.batches" :key="batch.id">
                        <div class="mb-2 last:mb-0">
                            <div class="flex items-center justify-between text-xs">
                                <span class="truncate font-medium text-gray-700 dark:text-gray-300" x-text="batch.name || 'Batch #' + batch.id.substring(0, 8)"></span>
                                <span class="ml-2 shrink-0 text-gray-400 dark:text-gray-500" x-text="batch.progress + '%'"></span>
                            </div>
            ...
            <template x-if="snapshot.running_jobs.length > 0">
                <div class="border-b border-gray-100 px-4 py-2 dark:border-white/10">
                    <p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{{ __('Running') }}</p>
                    <template x-for="job in snapshot.running_jobs" :key="job.id">
                        <div class="flex items-center justify-between py-0.5 text-xs">
                            <div class="flex min-w-0 items-center gap-1.5">
                                <svg class="h-3 w-3 shrink-0 animate-spin text-primary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                                </svg>
                                <span class="truncate font-medium text-gray-700 dark:text-gray-300" x-text="job.human_name || job.name || '{{ __('Unknown job') }}'"></span>
                            </div>
                            <div class="ml-2 flex shrink-0 items-center gap-2 text-gray-400 dark:text-gray-500">
                                ...
                                <span class="rounded bg-gray-100 px-1 dark:bg-white/10" x-text="job.queue || 'default'"></span>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <template x-if="snapshot.upcoming.length > 0">
                <div class="px-4 py-2">
                    <p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{{ __('Pending') }}</p>
                    <template x-for="job in snapshot.upcoming" :key="job.id">
                        <div class="flex items-center justify-between py-0.5 text-xs">
                            <span class="truncate text-gray-600 dark:text-gray-400" x-text="job.human_name || job.name || '{{ __('Unknown job') }}'"></span>
                            <span class="ml-2 shrink-0 rounded bg-gray-100 px-1 text-gray-400 dark:bg-white/10 dark:text-gray-500" x-text="job.queue || 'default'"></span>
                        </div>
                    </template>
                </div>
            </template>
